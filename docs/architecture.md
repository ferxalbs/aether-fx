...
Local sessions are versioned JSONL files under `<workspace>/.aether-fx/sessions/<session-id>.jsonl`. Schema version 3 stores `Started`, committed `TurnSnapshot` records, and `Finished`. Each completed turn is one JSONL record containing turn metadata, minimized context (paths/hashes/ranges and safe tool summaries), and sanitized continuation. Raw prompts, assistant text, excerpt bodies, and command output are not persisted. Session directories and files are created and opened without following symlinks or Windows reparse points and must remain inside the canonical workspace. `aether resume <session-id>` restores the last committed turn, re-hashes inspected files, and discards Rainy continuation when the workspace root or inspected files have changed. Unsupported older schemas fail closed. Truncated trailing records are ignored; corrupt replay never compact-overwrites the original file.
