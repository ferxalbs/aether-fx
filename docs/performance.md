...
Phase 2.1 is a session-safety closure: schema version 3 committed `TurnSnapshot` records, Unix session modes, continuation invalidation on workspace drift, and typed `InvalidContinuation` recovery. No new Criterion run was taken for this correctness pass. Session replay now validates schema v3 rather than v2.
